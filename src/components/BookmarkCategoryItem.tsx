...
const BookmarkCategoryItem: React.FC<Props> = (props) => {
  ...
  return (
    <li key={props.bookmark.id} className='flex flex-row items-center gap-4 group'>
      <a
      ...
      <FiTrash2 className='text-lg opacity-0 cursor-pointer transition-all duration-500 hover:text-primaryRed group-hover:opacity-90' onClick={deleteHandler} />
    </li>
  );
};
